...
namespace CUST.WKG
{
    public partial class ST_TKT_Detail : System.Web.UI.Page
    {
        ...
        protected void DateBind()
        {
            ...
            DataTable st = tkt.Select_TKT_Detail(st_id);
            lbl_tg.Text = st.Rows[0]["tg"].ToString().Replace("##&&##", "_______");
            string[] SplitStr = { "##&&##" };
            string[] das = st.Rows[0]["da"].ToString().Split(SplitStr, StringSplitOptions.RemoveEmptyEntries);
            string da = "";
            for (int i = 0; i < das.Length; i++)
            {
                da += "<font size=\"5px\">" + (i + 1) + "  . </font> " + das[i] + "   ";
            }
            lbl_da.Text = da;
