...
namespace CUST.WKG
{
    public partial class DQZC_DSZSAdd : System.Web.UI.Page
    {
        ...
        protected void btn_bc_Click(object sender, EventArgs e)
        {
            ...
            struct_DSZS.p_bt = tbx_bt.Text.ToString().Trim();
            struct_DSZS.p_nr = tbx_nr.Text.ToString().Trim();
            struct_DSZS.p_czsj = DateTime.Parse(DateTime.Now.ToString());
            struct_DSZS.p_fbip = Dns.GetHostEntry(Dns.GetHostName()).AddressList.FirstOrDefault<IPAddress>(a => a.AddressFamily.ToString().Equals("InterNetwork")).ToString(); ;
            struct_DSZS.p_fbr = lbl_fbr.Text;

            struct_DSZS.p_czfs = "02";
            struct_DSZS.p_czxx = "位置：员工管理->员工奖励->添加   描述：员工编号：" + _usState.userLoginName
                 + "起始日期：" + struct_DSZS.p_fbsjks + "截止日期：" + struct_DSZS.p_fbsjjs
                + "备注：";
            dszs.Insert_DSZS(struct_DSZS);
