...
namespace CUST.WKG
{
    public partial class SBBJ_CK : System.Web.UI.Page
    {
        public int cpage;
        public int psize;
        ...
        protected void show()
        {
            ...
            if (SysData.HasThisBMQX(_usState.userID, "140602"))
            {
        ...
        private void bind_select()
        {
            ...
            struct_bjck.p_zt = ddlt_zt.SelectedValue.Trim().ToString();
            ...
            {
                struct_bjck.p_kssj = Convert.ToDateTime(tbx_kssj.Text.ToString());
            }
            ...
            {
                struct_bjck.p_jssj = Convert.ToDateTime(tbx_jssj.Text.ToString());
            }

            int count = bj.Select_BJCK_Count(struct_bjck);
            pg_fy.TotalRecord = count;
            struct_bjck.p_currentpage = pg_fy.CurrentPage;
            struct_bjck.p_pagesize = pg_fy.NumPerPage;
        ...
        protected void btn_add_Click(object sender, EventArgs e)
        {
            ...
            Response.Redirect("../SheBei/BJCK_Add.aspx");
        }
        ...
        protected void Repeater1_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            ...
            if (e.CommandName == "Edit")
            {
                ...
                {
                    //查询是否有权限编辑
                    if (SysData.HasThisBMQX(_usState.userID, bmdm, "140603"))
                    {
                        Server.Transfer("BJCK_Edit.aspx?id=" + id + "&sjbs=" + sjbs + "&sjc=" + sjc);
                    }
                    else
                    {
            ...
            else if (e.CommandName == "Delete")
            {
                ...
                if (zt == "0" || zt == "4")
                {
                    ...
                    if (lrr.Equals(_usState.userLoginName))
                    {
                        ...
                        bj.Delete_BJCK(Convert.ToString(id), p_czfs, p_czxx);
                    }
                    else
                    {
                ...
                {
                    if (SysData.HasThisBMQX(_usState.userID, bmdm, "140604"))
                    {
                        string p_czfs = "04";
                        string p_czxx = "位置：员工奖励->员工奖励管理->奖励    描述：员工编号：[" + _usState.userLoginName + "]";
                        bj.Delete_BJCK(Convert.ToString(id), p_czfs, p_czxx);
            ...
            else if (e.CommandName == "Update_TJ")
            {
                ...
                if (zt == "0")
                {
                    ...
                    if (lrr.Equals(_usState.userLoginName))
                    {
                        struct_bjck.p_id = content[0];
                        struct_bjck.p_zt = "1";
                        struct_bjck.p_bhyy = HF_yc.Value;
                        struct_bjck.p_czfs = "05";
                        struct_bjck.p_czxx = "位置：员工奖励->状态->改变状态（提交）    描述：员工编号：[" + _usState.userLoginName + "]";
                        bj.Update_BJCKZT(struct_bjck);
                        //插入到HT_TSR表中
                        SysData.Insert_TSR(csr, "14", "1406", id);
                    }
                    else
                    {
            ...
            else if (e.CommandName == "Update_SH")
            {
                if (zt == "1")
                {
                    ...
                    if (csr.Equals(_usState.userLoginName))
                    {
                        ...
                        struct_bjck.p_zt = "2";
                        struct_bjck.p_bhyy = HF_yc.Value;
                        struct_bjck.p_czfs = "06";
                        struct_bjck.p_czxx = "位置：员工奖励->状态->改变状态（审核）    描述：员工编号：[" + _usState.userLoginName + "]";
                        bj.Update_BJCKZT(struct_bjck);
                        SysData.Delete_TSR(csr, "14", "1406", id);
                        SysData.Insert_TSR(zsr, "14", "1406", id);
                    }
                    else
                    {
                ...
                else if (zt == "2")
                {
                    ...
                    if (zsr.Equals(_usState.userLoginName))
                    {
                        ...
                        if (sjbs.Equals("0"))
                        {
                            struct_bjck.p_shsj = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss", DateTimeFormatInfo.InvariantInfo);
                            struct_bjck.p_bhyy = HF_yc.Value;
                            struct_bjck.p_czfs = "06";
                            struct_bjck.p_czxx = "位置：员工奖励->状态->改变状态（审核）    描述：员工编号：[" + _usState.userLoginName + "]";
                            bj.Update_BJCK_SJBS_ZT(struct_bjck);
                            SysData.Delete_TSR(zsr, "14", "1406", id);
                        }
                        else if (sjbs.Equals("2"))
                        {
                            ...
                            struct_bjck.p_bhyy = HF_yc.Value;
                            struct_bjck.p_czfs = "06";
                            struct_bjck.p_czxx = "位置：员工奖励->状态->改变状态（审核） 将原最终数据变为历史数据   描述：员工编号：[" + _usState.userLoginName + "]";
                            bj.Update_BJCK_SJBS_LSSJ_ZT(struct_bjck);

                            //将副本数据变为最终数据
                            struct_bjck.p_shsj = DateTime.Now.ToString("yyyy-MM-dd HHo:mm:ss", DateTimeFormatInfo.InvariantInfo);
                            struct_bjck.p_bhyy = HF_yc.Value;
                            struct_bjck.p_czfs = "06";
                            struct_bjck.p_czxx = "位置：员工奖励->状态->改变状态（审核）将原副本数据变为最终数据    描述：员工编号：[" + _usState.userLoginName + "]";
                            bj.Update_BJCK_SJBS_FBSJ_ZT(struct_bjck);
                            SysData.Delete_TSR(zsr, "14", "1406", id);
            ...
            else if (e.CommandName == "Update_BH")
            {
                if (zt == "1")
                {
                    ...
                    if (csr.Equals(_usState.userLoginName))
                    {
                        ...
                        struct_bjck.p_zt = "4";
                        struct_bjck.p_bhyy = HF_yc.Value;
                        struct_bjck.p_czfs = "07";
                        struct_bjck.p_czxx = "位置：员工奖励->状态->改变状态（驳回）    描述：员工编号：[" + _usState.userLoginName + "]";
                        bj.Update_BJCKZT(struct_bjck);
                        SysData.Delete_TSR(csr, "14", "1406", id);
                    }
                    else
